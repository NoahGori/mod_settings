...
public:
  inline static RED4ext::rtti::IType *GetType() { return s_type; }

protected:
  inline static RED4ext::rtti::IType *s_type;
};
...
template <typename T>
concept IsTypeInfoOrVoid = IsCompatible<T, RED4ext::rtti::IType> or std::is_same_v<void, T>;
...
template <typename T> struct Specialization : public std::false_type {};

template <template <typename> class G, typename A> struct Specialization<G<A>> : public std::true_type {
...
template <typename T> struct RedSpecialization : public std::false_type {};

template <typename A> struct RedSpecialization<RED4ext::DynArray<A>> : Specialization<RED4ext::DynArray<A>> {
  static constexpr auto prefix = "array:";
};

template <typename A> struct RedSpecialization<RED4ext::Handle<A>> : Specialization<RED4ext::Handle<A>> {
  static constexpr auto prefix = "handle:";
};

template <typename A> struct RedSpecialization<RED4ext::WeakHandle<A>> : Specialization<RED4ext::WeakHandle<A>> {
  static constexpr auto prefix = "whandle:";
};

template <typename A> struct RedSpecialization<RED4ext::ScriptRef<A>> : Specialization<RED4ext::ScriptRef<A>> {
...
template <typename F> struct FunctionPtr : public std::false_type {};

template <typename R, typename... Args> struct FunctionPtr<R (*)(Args...)> : public std::true_type {
  using context_type = void;
  using return_type = R;
  using arguments_type = std::tuple<std::remove_const_t<std::remove_reference_t<Args>>...>;
  template <size_t I> using argument_type = typename std::tuple_element<I, arguments_type>::type;
  using qualified_arguments_type = std::tuple<Args...>;
  template <size_t I> using qualified_argument_type = typename std::tuple_element<I, qualified_arguments_type>::type;
...
template <typename T> consteval const auto &GetSelfSignature() {
#ifdef _MSC_VER
  return __FUNCSIG__;
#else
...
template <typename T> consteval std::pair<size_t, size_t> GetTypeNameBounds() {
...
template <typename T> consteval auto GeFullTypeNameArray() {
...
template <typename T> consteval auto GetShortTypeNameArray() {
  using U = std::remove_pointer_t<std::remove_cvref_t<T>>;
...
template <typename T> const char *ExtractFullTypeName() {
  static auto name = TypeNameExtractor::GeFullTypeNameArray<T>();
  return name.data();
}

template <typename T> const char *ExtractShortTypeName() {
  static auto name = TypeNameExtractor::GetShortTypeNameArray<T>();
  return name.data();
}

template <typename T, RED4ext::CName N> struct TypeCase {
  using type = T;
  static constexpr auto name = N;
};

template <typename T, typename Case> consteval bool TryTypeCase(RED4ext::CName &name) {
...
template <typename T, typename... Cases> consteval RED4ext::CName MatchTypeCase() {
  RED4ext::CName name{};
  (TryTypeCase<T, Cases>(name) || ...);
  return name;
}

template <typename T, RED4ext::CName Prefix = 0xCBF29CE484222325> consteval RED4ext::CName ResolveConstTypeName() {
  ...
  return MatchTypeCase<
      T, TypeCase<int8_t, RED4ext::FNV1a64("Int8", Prefix)>, TypeCase<uint8_t, RED4ext::FNV1a64("Uint8", Prefix)>,
      TypeCase<int16_t, RED4ext::FNV1a64("Int16", Prefix)>, TypeCase<uint16_t, RED4ext::FNV1a64("Uint16", Prefix)>,
      TypeCase<int32_t, RED4ext::FNV1a64("Int32", Prefix)>, TypeCase<uint32_t, RED4ext::FNV1a64("Uint32", Prefix)>,
      TypeCase<int64_t, RED4ext::FNV1a64("Int64", Prefix)>, TypeCase<uint64_t, RED4ext::FNV1a64("Uint64", Prefix)>,
      TypeCase<float, RED4ext::FNV1a64("Float", Prefix)>, TypeCase<double, RED4ext::FNV1a64("Double", Prefix)>,
      TypeCase<bool, RED4ext::FNV1a64("Bool", Prefix)>, TypeCase<RED4ext::CString, RED4ext::FNV1a64("String", Prefix)>,
...
template <typename T> std::string BuildDynamicTypeName() {
...
template <typename T> constexpr RED4ext::CName ResolveTypeName() {
  using U = std::remove_pointer_t<std::remove_cvref_t<T>>;
...
template <typename T> inline void ExtractArg(RED4ext::CStackFrame *aFrame, T *aArg) {
...
template <typename... Args> inline void ExtractArgs(RED4ext::CStackFrame *aFrame, std::tuple<Args...> &aArgs) {
...
template <typename R, typename... Args> using StaticFunction = R (*)(Args...);

template <class C, typename R, typename... Args> using MemberFunction = R (C::*)(Args...);
...
inline RED4ext::ScriptingFunction_t<void *> WrapScriptableFunction() {
  ...
  auto f = [](RED4ext::IScriptable *aContext, RED4ext::CStackFrame *aFrame, R *aRet, int64_t aRetType) -> void {
    Args args;
