...
Engine::RTTIRegistrar::RTTIRegistrar(CallbackFunc aRegFunc, CallbackFunc aBuildFunc)
    : m_registered(false), m_regFunc(aRegFunc), m_buildFunc(aBuildFunc) {
  s_pending.push_back(this);
}

void Engine::RTTIRegistrar::Register() {
  if (!m_registered) {
    RED4ext::CRTTISystem::Get()->AddRegisterCallback(m_regFunc);
    RED4ext::CRTTISystem::Get()->AddPostRegisterCallback(m_buildFunc);
    m_registered = true;
  }
}
