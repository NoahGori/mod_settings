...
public:
  ...
  static void ReadValueFromFile(ScriptProperty *prop, void *pointer);

  template <typename T> static void ReadValueFromFile(CName className, CName propertyName, T *pointer) {
    auto self = ModSettings::GetInstance();
    CString settingFromFile;
    if (ModSettings::GetSettingString(className, propertyName, &settingFromFile)) {
  ...
  static_assert(Detail::IsAllocator<Memory::RTTIAllocator>, "Memory::RTTIAllocator"
                                                            " is not a valid allocator type");

private:
  // std::shared_mutex variables_lock;
  RED4ext::SharedSpinLock variables_lock;
  std::map<uint64_t, WeakHandle<IScriptable>> listeners;
